https://kitboga.com/codejam26

1.) ADVERTISEMENT
- Billy's Cheeseballs, I will ramble about cheeseballs for like 10 minutes
	- Timer: you can skip ad after 30 seconds, but there on the first chance to break.
	- buffering
	- yellow ad bar for the video
2.) TIME WASTER
* 
* Quick Time Event: during the video press a certain key
* INTERACTIVE VIDEO W/ AD
	* Starts - Answer this quick survey to skip the video!
		*  rank this ad from 1 - 5 stars!
			* no matter what they click, it becomes 5 stars
		* We see you loved this ad! Please tell us more about it!
			* 200 word minimum
		* Click on where the dog would be happiest!
			* all pictures of dog parks, completely subjective 
		* Click on where the cat would be happiest!
			* all pictures of places cats love
			* 7-9 auto 
		* What animal is this?
			* changing animals 
		* Fake website behind skip video survey
			* Billy's Cheeseballs
3.) ENDING
 * Loading bar: whenever we finish the annoying UX, we have a bar which will attempt to load, which never finishes.
 * If they watch the whole ad: we make it buffer at like 9:45 will buffer to 9:55 and stop.
	